...
| 3.9 | Read the account nonce at `pending` rather than `latest`. | **DONE** — every settle and every funding was failing `DuplicateNonce` on `0x2cf`, three attempts each, every cycle. `getNonce()` defaults to the confirmed state and ignores the mempool, so anything else signing from the account — the previous replica during a rolling deploy — makes the keeper pick a nonce already claimed, and the retry re-reads the same stale value for ever |
