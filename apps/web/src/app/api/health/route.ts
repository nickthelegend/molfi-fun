...
export async function GET() {
   ...
   *
   * Shared with `/api/door`, which the gate calls: two callers, one rule, no drift. Everything
   * above can be green while the product is unusable by anyone who does not already have an
   * account, and nothing here asked that until a visitor found out by signing in.
   */
  const door: Part = await readDoor();
